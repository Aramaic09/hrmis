...
function showLeaves() 
{
   $leave = App\Leave_type::whereIn('id',[1,2,3,5,6,11])->orderBy('leave_desc')->get();
...
{
   $leave = App\Leave_type::whereIn('id',[1,2,3,5,6,11])->get();
...
function getLeaves($user_id,$leave_id) {
	if($leave_id == 5)
	{	
		$leave = collect(App\Employee_cto::where('user_id',$user_id)->where('cto_bal','>',0)->orderBy('id','DESC')->first());
		if($leave->all())
		{
			return $leave['cto_bal'] + 1.25;
		}
		else
		{
			return "0";
	...
	{
		$leave = collect(App\Employee_leave::where('user_id',$user_id)->where('leave_id',$leave_id)->orderBy('created_at','DESC')->first());
		$leave = $leave->all();
...
{
	return App\View_user::where('division',Auth::user()->division)->whereIn('employment_id',[1,13,14])->orderBy('lname','ASC')->get();
}

function getStaffDivision2($div)
{
	return App\View_user::where('division',$div)->whereIn('employment_id',[1,13,14])->orderBy('lname','ASC')->get();
}
...
function getFileForApplicant($id)
{
	...
	//GET LAST FILE
	$file = App\Recruitment_file_history::where('request_id',$letter_id['id'])->where('file_type','Vacancy Advise')->orderBy('id','desc')->first();

...
class RequestForHiringController extends Controller
{
    ...
    public function upload($type)
    {
        ...
        App\Request_for_hiring::where('id',request()->tblid)
                        ->update([
                                    'request_status' => "Vacancy Advise Uploaded",
                                ]);

        //FILE HISTORY FOR RECRUITMENT
        $file = new App\Recruitment_file_history;
        $file->request_id = request()->tblid;
        $file->file_type = "Vacancy Advise";
        $file->file_path = $path;
        $file->save();
        ...
        $history->request_status = 'Division sent Vacancy Advise';
        $history->created_by = getDivision(Auth::user()->division);
        $history->save();  
    ...
    public function approval()
    {
        switch (Auth::user()->division) {
            //BUDGET
            case 'q':
                    $list = App\Request_for_hiring::where('request_status','For FAD-Budget clearance')->get();
                break;
            //OED-ARMMS
            case 'A':
                    $list = App\Request_for_hiring::where('request_status','For OED-ARMSS clearance')->get();
                break;
            //OED
            case 'O':
                    $list = App\Request_for_hiring::where('request_status','For OED`s Approval')->get();
                break;
    ...
    public function clearance()
    {
        switch (Auth::user()->division) {
            //BUDGET
            case 'q':
                    $history_status = "Received and cleared, forwarded to OED-ARMSS";
                    $status = 'For OED-ARMSS clearance';
                break;
            //OED-ARMMS
            case 'A':
                    $history_status = "Received and cleared, forwarded to OED";
                    $status = "For OED`s Approval";
                break;
            //OED
            case 'O':
                    $history_status = "Received and approved, upload Vacancy Advise";
                    $status = 'Upload Vacancy Advise';
                break;
